...
{
	fprintf( stderr, "gcmorse " VERSION " by " AUTHOR "\n\n\r" );
	fprintf( stderr,
...
void help( int exitcode )
{
	...
	fprintf( stderr, "Usage: gcmorse [OPTIONS]\n\r" );
...
void encrypt( FILE *outputfile, char *str, size_t len )
{
	...
	for ( i = 0; i < len; i++ )
	{
		...
		if ( badc ) fprintf( stderr, "gcmorse: unsupported character (ASCII only) - %c (0x%x) - c%ld\n\r", str[i], str[i], (long) i );
	}
}
...
void decrypt( FILE *outputfile, char *str, size_t len )
{
	...
	for ( i = 0; i < len; i++ )
	{
		...
		if ( badc ) fprintf( stderr, "gcmorse: unsupported Morse code - '%s' - c%ld\n\r", morsechar, (long) i );
...
int main( int argc, char **argv )
{
	...
	for ( i = 1; i < argc; i++ )
	{
		...
		if ( !strcmp( argv[i], "-i" ) || !strcmp( argv[i], "--input" ) )
		{
			...
			{
				fprintf( stderr, "gcmorse: missing input file name.\nTry -h option to get more information.\n\r" );
				exit( 1 );
			}
		}
		...
		if ( !strcmp( argv[i], "-o" ) || !strcmp( argv[i], "--output" ) )
		{
			...
			{
				fprintf( stderr, "gcmorse: missing output file name.\nTry -h option to get more information.\n\r" );
				exit( 1 );
			}
		}
		...
		if ( !argparsed )
		{
			fprintf( stderr, "gcmorse: unknown option: %s\n\r", argv[i] );
			help( 1 );
		}
	}
	...
	{
		fprintf( stderr, "gcmorse: unable to open input file.\nTry -h option to get more information.\n\r" );
		exit( 1 );
	}
	...
	{
		fprintf( stderr, "gcmorse: unable to open output file.\nTry -h option to get more information.\n\r" );
		exit( 1 );
	}
